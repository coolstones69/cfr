...
static int * chanceOutcomes1 = nullptr;
//static int * chanceOutcomes2 = NULL;
static int * chanceOutcomes2 = nullptr;
...
void unrankco(int i, int * roll, int player)
{
  ...
  vector<int> chanceOutcomes;
  if (player == 1){
	  chanceOutcomes.push_back(reinterpret_cast<int>(chanceOutcomes1));
  }
  else {
	  chanceOutcomes.push_back(reinterpret_cast<int>(chanceOutcomes2));
  }
...
void nextRoll(vector<int>  roll, int size)
{
...
int getRollBase10(vector<int> roll, int size)
{
  int multiplier = 1;
  int val = 0;
  for (int i = size-1; i >= 0; i--)
...
void determineChanceOutcomes(int player)
{
  ...
  for (p = 0; p < permutations; p++) {
	...
	rollcopy.resize(rolls.size());
	copy(rolls.begin(), rolls.end(), rollcopy.begin());
  ...
  chanceOutcomes.pop_back();
  chanceOutcomes.push_back(numChanceOutcomes);
